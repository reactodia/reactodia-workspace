...
type DefaultPropKeys =
    ...
    | 'language';
type ProvidedProps =
    Omit<WorkspaceProps, DefaultPropKeys> &
    Required<Pick<WorkspaceProps, DefaultPropKeys>>;
...
export class Workspace extends Component<WorkspaceProps, State> {
    static readonly defaultProps: Required<Pick<WorkspaceProps, DefaultPropKeys>> = {
        hideTutorial: true,
    ...
    private markup: WorkspaceMarkup | undefined | null;
    ...
    constructor(props: WorkspaceProps) {
        ...
        const {
            hideHalo, history, viewOptions = {}, language,
    ...
    _getPaperArea(): PaperArea | undefined | null {
    ...
    render(): ReactElement<any> {
        ...
        return createElement(WorkspaceMarkup, {
            ref: this.onMountWorkspaceMarkup,
            hidePanels,
            hideToolbar,
            hideScrollBars,
            model: this.model,
    ...
    private onMountWorkspaceMarkup = (markup: WorkspaceMarkup | null) => {
        this.markup = markup;
    };
    ...
    componentDidMount() {
        ...
        const paperArea = this.markup!.paperArea!;
        this.editor._initializePaperComponents(paperArea);
    ...
    componentWillReceiveProps(nextProps: ProvidedProps) {
    ...
    showWaitIndicatorWhile(operation: Promise<any>) {
        ...
        if (operation) {
            operation.then(() => {
                ...
                console.error(error);
                this.editor.setSpinner({statusText: 'Unknown error occurred', errorOccurred: true});
            });
        }
    }
    ...
    exportPng = (fileName?: string) => {
        this.markup!.paperArea!.exportPNG({backgroundColor: 'white'}).then(dataUri => {
            ...
            saveAs(blob, fileName || 'diagram.png');
        });
    }
